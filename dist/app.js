...
exports.socket = exports.server = exports.sequelize = exports.redisServer = void 0;
...
const app = (0, _express.default)(); // const cookieParser = require("cookie-parser");
// const csrf = require("csurf");
// const admin = require("firebase-admin");

const server = _http.default.createServer(app); // const csrfMiddleware = csrf({ cookie: true });
...
const socket = async () => {
  ...
  var clients = {};
  var clientsId = {}; // Run when client connects

  io.on('connection', socket => {
    const subscriber = redisServer.duplicate();
    subscriber.connect();
    console.log('socket connected'); // redisServer.pSubscribe('notification-*', (message, channel) => {
    ...
    socket.on('signin', async data => {
      ...
      await (0, _location.getProfile)(data.id, async (username, profileImage) => {
        console.log("sadfsadfsadfsadfsadfasd===========", username, profileImage);
        oldValue.name = username;
        oldValue.image = profileImage;
      });
      await redisServer.set(chanelName, JSON.stringify({ ...oldValue
      }), function (err, reply) {
        if (err) {
      ...
      (0, _location.getFriends)(data.id, async (person, friends) => {
        ...
        for (let i = 0; i < friends.length; i++) {
          ...
          if (friendSocket) {
            // join friends room 
            socket.join(`location${friends[i].uid}`); //add friends to your room
      ...
      (0, _location.getGroups)(data.id, async (owner, groups, groupsUnblocked) => {
        console.log("every thing went according to plan"); //join friends room for loop
        // console.log("iterating over the fore loop");
        ...
        if (type) {
          if (createGroup) {
            if (friendVisibility) {
              ...
              io.to(friendSocket.id).emit("friend-visibility", {
                fuId: obj.userId,
                fid: obj.sId,
                online: createGroup
              });
            ...
            io.to(friendSocket.id).emit("location", {
              id: obj.userId,
              type: 0,
              online: createGroup
            });
            io.to(userSocket.id).emit("location", {
              id: obj.friendId,
              type: 0,
              online: createGroup
            });
      ...
      subscriber.subscribe(`group-location-s`, async (message, channel) => {
        ...
        if (type) {
          ...
          if (allowGroup) {
            ...
            userSocket.to(groupSocket.toString()).emit("location", {
              id: obj.userId,
              type: 1,
              groupOnline: allowGroup,
              group: groupSocket.toString()
            });
            userSocket.emit("location", {
              clearGroup: true,
              group: groupSocket.toString()
            });
          ...
          userSocket.join(groupSocket.toString());
          socket.to(groupSocket.toString()).emit('location', JSON.parse(userLocation));
          console.log("this is the the group initial creation or joining =========-->>>>>>>>>>", groupSocket); //add to list of groups aligable for location brodcast
    ...
    socket.on('location', async msg => {
      ...
      msg.type = 0;
      msg.group = ""; // console.log("data",msg);
      ...
      await redisServer.set(chanelName, JSON.stringify({ ...msg
      }), function (err, reply) {
        if (err) {
          console.log("error", err);
        }
      ...
      socket.to(clientRoom.toString()).emit('location', msg); //emite to groups
    ...
    socket.on('logout', data => {
      ...
      subscriber.unsubscribe(`notification-${data.id}`); // subscriber.unsubscribe(`notification-${data.id}`, (message, channel) => {
      //   console.log('CHANNEL UNSUBSCRIBED');
      //   console.log('Message ', message);
      //   console.log('Channel ', channel);
      // });
    });
    subscriber.on('error', err => console.log('ERR >> ', err)); // Runs when client disconnects
    ...
    socket.on('disconnect', async data => {
      ...
      if (clientsId[socket.id]) {
        ...
        if (value) {
          ...
          await redisServer.set(chanelName, JSON.stringify({ ...value
          }), function (err, reply) {
            if (err) {
              console.log("error", err);
            }
        ...
        (0, _location.signOutVisibility)(Id);
        delete clients[Id];
        delete clientsId[socket.id];
        const clientRoom = `location${Id}`;
        socket.to(clientRoom.toString()).emit('location', value); // io.emit('location', msg);
      }
    });
  });
...
app.use(_express.default.static(_path.default.join(__dirname, '../public/uploads')));
