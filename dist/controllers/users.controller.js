...
exports.getAll = async (req, res, next) => {
  try {
    const ALL = await User.findAll({
      include: ['userFriends', 'friends', 'groups', 'announcements', 'uannouncements', {
        model: Group,
        as: 'ugroups',
        include: {
          model: User,
...
exports.getSuggestion = async (req, res, next) => {
  try {
    ...
    const friendsArr = [...friends.friends, ...friends.userFriends].map(fri => fri.uid);
    ...
    res.status(200).json(users.filter(user => {
      const truthArr = [...user.friends, ...user.userFriends].map(fri => fri.uid === req.uid ? true : false);
    ...
    try {
      const user = await User.findAll({
        ...
        include: [{
          ...
          where: {
            username: {
              [Op.like]: `%${req.query.key.toLowerCase()}%`
            }
          },
          through: {
            where: {
...
exports.proccessRequest = async (req, res, next) => {
  try {
    ...
    if (req.query.type === _constants.RequestProcessType.ACCEPT_REQUEST) {
      (0, _notification.requestAccepted)(friend.uid, user.username);
      (0, _notification.requestAcceptedLocation)(friend.uid, req.uid, req.body.friendId, user.id, true, 0, true);
      user.addUserFriends(req.body.friendId, {
        through: {
          isAccepted: true
        }
      });
    } else if (req.query.type === _constants.RequestProcessType.REJECT_REQUEST) {
      (0, _notification.requestRejected)(friend.uid, user.username);
      user.removeUserFriends(req.body.friendId);
    } else if (req.query.type === _constants.RequestProcessType.SEND_REQUEST) {
      (0, _notification.requestSent)(friend.uid, user.username);
...
exports.changeVisibility = async (req, res, next) => {
  try {
    ...
    const bool = req.query.type === _constants.ChangeVisibilityType.ACCEPT_VISIBLE ? true : req.query.type === _constants.ChangeVisibilityType.REJECT_VISIBLE ? false : null;
    console.log("we here with data", req.body.friendId, req.body, req.query.type, user.id);
    let data;
    let friendVisibility;

    if (req.body.userType == 1) {
      friendVisibility = await UserFriend.findOne({
        where: {
          user_id: user.id,
    ...
    (0, _notification.requestAcceptedLocation)(friend.uid, req.uid, req.body.friendId, user.id, bool, 1, friendVisibility); //update database

    if (req.body.userType == 1) {
      data = await UserFriend.update({
        userVisible: bool
      }, {
        where: {
          user_id: user.id,
    ...
    console.log("all information", bool, data); // if (req.query.type === ChangeVisibilityType.ACCEPT_VISIBLE) {
...
exports.processGroupRequest = async (req, res, next) => {
  try {
    ...
    if (req.query.type === _constants.GroupRequestProcessType.ACCEPT_GROUP_REQUEST) {
      (0, _notification.groupRequestLocation)(req.body.groupId, group.owner_id, req.uid, true, 0);
      user.addGroups(req.body.groupId, {
        through: {
          isAccepted: true
        }
      });
    } else if (req.query.type === _constants.GroupRequestProcessType.REJECT_GROUP_REQUEST) {
...
exports.createOne = async (req, res, next) => {
  ...
  try {
    ...
    if (existingUserName.length > 0 || existingEmail.length > 0) {
      ...
      const user = await User.create(userdata); // userlocationdata.owner_id=user.id;

      console.log("user", user); // const userlocation = await Location.create(userlocationdata);
      // console.log("userlocation",userlocation);
      // user.addFriend(req.body.friendId, { through: { isAccepted: true } });

      if (req.body.friend) {
        const singleById = await User.findByPk(req.body.friend);
        user.addFriends(singleById, {
          through: {
            isAccepted: false
          }
        });
...
exports.update = async (req, res, next) => {
  try {
    ...
    if (!(0, _helper.isNUll)(req.body.username)) userData.username = req.body.username;
    if (!(0, _helper.isNUll)(req.body.bio)) userData.bio = req.body.bio;
