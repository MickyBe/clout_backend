...
exports.create = async (req, res, next) => {
  try {
    ...
    if (req.body.users) {
      group.addMembers(req.body.users, {
        through: {
          isAccepted: false
        }
      });
    ...
    (0, _notification.groupRequestLocation)(group.id, data.owner_id, req.uid, true, 0);
    (0, _notification.groupRequest)(members.map(member => member.uid), req.body.name, owner.username);
...
exports.changeVisibilityGroup = async (req, res, next) => {
  try {
    ...
    const bool = req.query.type === _constants.ChangeGroupVisibilityType.ACCEPT_VISIBLE ? true : req.query.type === _constants.ChangeGroupVisibilityType.REJECT_VISIBLE ? false : null;
    ...
    (0, _notification.groupRequestLocation)(req.body.groupId, req.body.ownerId, req.uid, bool, 1); //update database

    if (user.id != req.body.ownerId) {
      data = await UserGroup.update({
        userVisible: bool
      }, {
        where: {
          UserId: user.id,
...
exports.addMem = async (req, res, next) => {
  try {
    ...
    const member = await User.findByPk(req.body.memberId);
    (0, _notification.groupRequest)([member.uid], group.name, group.owner.username);
    return res.status(200).send(true);
  } catch (err) {
    console.log(err);
    return res.status(500).json(err);
